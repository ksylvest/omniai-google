    ...
    private
      ...
      def process_sync!
        ...
        unless response.status.ok?
          ...
          if error_data.dig("error", "message")&.include?("60 seconds")
        ...
        transformed_data = { "text" => transcript }
      ...
      def process_async!
        ...
        # Transform Google's response to match expected format
        transformed_data = if transcript_data.is_a?(Hash)
                             # VERBOSE_JSON format with detailed timing info
                             transcript_data
                           else
                             # Simple text format
