...
RSpec.describe OmniAI::Google::Chat do
  ...
  describe '.process!' do
    ...
    context 'when formatting as JSON' do
      ...
      before do
        stub_request(:post, "https://generativelanguage.googleapis.com/v1/models/#{model}:generateContent?key=sse&key=...")
          .with(body: {
            generationConfig: { response_mime_type: 'application/json' },
            contents: [
              { role: 'user', parts: [{ text: OmniAI::Chat::JSON_PROMPT }] },
              { role: 'user', parts: [{ text: 'What is the name of the dummer for the Beatles?' }] },
            ],
          })
          .to_return_json(body: {
            candidates: [{
